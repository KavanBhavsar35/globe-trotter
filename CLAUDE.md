...
Building 11 of 13 screens. **Cut**: admin dashboard (#13), calendar/drag-reorder (#10 → list + up/down reorder). Profile settings (#12) now built: editable name/email/phone/city/country/bio (`PATCH /auth/me`), photo upload, delete account (`DELETE /auth/me`, cascades trips), saved-destinations list (`GET /auth/me/destinations`, derived from trip stops), language preference (localStorage `gt_lang` — preference-only, no i18n consumer).
