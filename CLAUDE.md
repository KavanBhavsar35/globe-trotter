...
- **Frontend**: Next.js **16** (App Router, `src/`) + React **19** + Tailwind **v4** + shadcn **"base-nova"** components.
  - shadcn base-nova is built on **Base UI** (`@base-ui/react`), **not Radix**. Controlled props are `open`/`onOpenChange` (Dialog) and `value`/`onValueChange` (Tabs). Some components use a `render={<El/>}` prop instead of Radix `asChild`.
...
- Auth: JWT + email in `localStorage` (`gt_token`, `gt_email`). All requests go through `src/lib/api.ts:apiFetch`; auth flows in `src/lib/auth.ts`. **localhost-demo only** — not XSS-safe for production.
- Routes: `/login` (public), everything under `src/app/(app)/*` is guarded by `useRequireAuth`, `/trip/[token]` is the public share view.
